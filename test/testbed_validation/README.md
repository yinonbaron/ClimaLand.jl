...
The selected fallback is the committed productive-cell fixture in
`fixtures/corpse_c_fresh_cell_11060`. It is regenerated by
`corpse-fortran-fixture` from pinned source, the documented CORPSE namelist,
and the source-exact GSWP3 cell-11060 driver. The fixture contains 365 daily
CORPSE records, its companion CASA record, and the cohort-resolved final
restart. The old mean remains informational unless its missing inputs are
recovered.
