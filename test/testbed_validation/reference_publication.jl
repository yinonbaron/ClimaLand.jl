...
function validate_expected_manifest(manifest, binding, kind, bindings; model = nothing)
    description = isnothing(model) ? "forcing expected manifest" :
    ...
    download isa AbstractDict && get(download, "url", nothing) == url &&
