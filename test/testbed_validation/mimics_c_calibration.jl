...
function calibration_method(; budget_absolute_only = false)
    selection = "choose the smallest r >= 0 minimizing a(r) + r*mean(x)"
    budget_absolute_only &&
        (selection *= "; use r = 0 for the zero-centered budget residual")
    return Dict(
...
function calibrated_envelope(actual, expected; relative = true)
    ...
    relative_value = 0.0
    if relative && right_derivative(relative_value, errors, references, 0.0) < 0
        upper = eps(Float64)
    ...
    absolute = max(0.0, maximum(errors .- relative_value .* references))
...
function signed_error_summary(actual, expected)
    errors = Float64.(actual) .- Float64.(expected)
    return Dict(
        "minimum" => minimum(errors),
...
function spatial_summary(errors, observations)
    isnothing(observations) && return Dict{String, Any}()
    sums = Dict{Int, Float64}()
    counts = Dict{Int, Int}()
    coordinates = Dict{Int, Tuple{Float64, Float64}}()
    for index in eachindex(errors)
        observation = observation_record(observations, index)
        all(
            field -> haskey(observation, field),
            ("cell_id", "latitude", "longitude"),
        ) || return Dict{String, Any}()
        cell_id = Int(observation["cell_id"])
        sums[cell_id] = get(sums, cell_id, 0.0) + errors[index]
        counts[cell_id] = get(counts, cell_id, 0) + 1
        coordinates[cell_id] = (
            Float64(observation["latitude"]),
            Float64(observation["longitude"]),
        )
    end
    means = Dict(id => sums[id] / counts[id] for id in keys(sums))
    maximum_cell = argmax(means)
    latitude, longitude = coordinates[maximum_cell]
    return Dict(
        "cell_count" => length(means),
        "mean_cell_absolute_error" => Statistics.mean(values(means)),
        "maximum_cell_mean_absolute_error" => means[maximum_cell],
        "maximum_cell_mean_absolute_error_cell_id" => maximum_cell,
        "maximum_cell_mean_absolute_error_latitude" => latitude,
        "maximum_cell_mean_absolute_error_longitude" => longitude,
    )
end
...
function calibration_record(
    ...
    return Dict(
        ...
        "signed_error" => signed_error_summary(actual, expected),
        "spatial_summary" => spatial_summary(envelope.errors, observations),
        "relative_error" => Dict(
...
function validate_distribution(values, location; allow_empty = false)
    ...
    Set(String.(keys(values))) == DISTRIBUTION_KEYS && all(
        value -> value isa Real && isfinite(value) && value >= 0,
        Base.values(values),
...
function validate_record(record, location, pair_count, observation_fields)
    ...
    signed = record["signed_error"]
    Set(String.(keys(signed))) ==
    Set(("minimum", "median", "maximum", "mean_bias")) &&
        all(isfinite, values(signed)) &&
        signed["minimum"] <= signed["median"] <= signed["maximum"] &&
        signed["minimum"] <= signed["mean_bias"] <= signed["maximum"] ||
        error("$location has an invalid signed-error summary")
    spatial = record["spatial_summary"]
    Set(String.(keys(spatial))) == Set((
        "cell_count",
        "mean_cell_absolute_error",
        "maximum_cell_mean_absolute_error",
        "maximum_cell_mean_absolute_error_cell_id",
        "maximum_cell_mean_absolute_error_latitude",
        "maximum_cell_mean_absolute_error_longitude",
    )) || error("$location has an incomplete spatial summary")
    0 < spatial["cell_count"] <= pair_count &&
        spatial["maximum_cell_mean_absolute_error_cell_id"] isa Integer &&
        all(
            value -> value isa Real && isfinite(value),
            (
                spatial["mean_cell_absolute_error"],
                spatial["maximum_cell_mean_absolute_error"],
                spatial["maximum_cell_mean_absolute_error_latitude"],
                spatial["maximum_cell_mean_absolute_error_longitude"],
            ),
        ) &&
        0 <=
        spatial["mean_cell_absolute_error"] <=
        spatial["maximum_cell_mean_absolute_error"] ||
        error("$location has an invalid spatial summary")
    ...
    for outlier in top
        ...
        relative_error = outlier["relative_error"]
        (
            relative_error == "undefined_zero_reference" ||
            relative_error isa Real &&
            isfinite(relative_error) &&
            relative_error >= 0
        ) || error("$location has an invalid relative outlier")
    end
...
function validate_provenance(document, required, generator)
    ...
    validate_digest(provenance["calibration"], "source_provenance.calibration")
    ...
    open(joinpath(@__DIR__, generator)) do io
        bytes2hex(SHA.sha256(io))
    end || error("MIMICS-C calibration generator hash is stale")
    provenance["calibration"]["sha256"] ==
    open(joinpath(@__DIR__, "mimics_c_calibration.jl")) do io
        bytes2hex(SHA.sha256(io))
    end || error("MIMICS-C calibration helper hash is stale")
    return nothing
end
...
function comparison_policy(boundary_path, historical_path)
    ...
    get(boundary, "model", nothing) == "MIMICS-C" ||
    ...
    return Dict(
        ...
        "fresh_fortran_budget" => Dict(
            "historical_residual_kg_c" => policy_record(
                budget_value,
                "budget.historical_residual_kg_c",
            ),
        ),
